...
// Colours are stored byte-swapped because the framebuffer is byte-swapped: the
// panel wants big-endian RGB565, and holding the frame that way lets it go out
// by DMA untouched instead of being copied pixel by pixel through a bounce
// buffer on the way. See panelFlush().
//
// The cost is this file. A native-endian colour reaching the framebuffer shows
// up as a wrong hue, so no colour is written inline anywhere else.
