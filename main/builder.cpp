...
static const char *SLOTS_NAMESPACE = "builder";
...
// Written by the CHIP task and by console commands, read by the ui task.
...
uint8_t builderType(uint8_t slot) {
  return builderSlotUsed(slot) ? s_slot[slot].type : (uint8_t)ACC_BUTTON;
}
...
static void persistSlots() {
  if (s_count == 0) {
    nvs_erase_key(s_nvs, SLOTS_KEY);
  } else {
    nvs_set_blob(s_nvs, SLOTS_KEY, s_slot, (size_t)s_count * sizeof(Slot));
  }
  nvs_commit(s_nvs);
}
...
void builderNudgeValue(uint8_t slot, int8_t direction) {
  ...
  next = std::clamp(next, (int32_t)accessoryMin(type), (int32_t)accessoryMax(type));
...
static void nudgeLevelSlot(uint8_t slot, int8_t direction) {
  ...
  builderSetLevel(slot, levelOf((uint8_t)std::clamp<int16_t>(percent, 0, 100)));
}
...
static bool loadSlots() {
  size_t stored = 0;
  if (nvs_get_blob(s_nvs, SLOTS_KEY, nullptr, &stored) != ESP_OK) {
    return false;
  }
  if (stored < sizeof(Slot) || stored > sizeof(s_slot) || stored % sizeof(Slot) != 0) {
    return false;
  }
  if (nvs_get_blob(s_nvs, SLOTS_KEY, s_slot, &stored) != ESP_OK) {
    return false;
  }
...
void builderBegin() {
  ESP_ERROR_CHECK(nvs_open(SLOTS_NAMESPACE, NVS_READWRITE, &s_nvs));
  loadSlots();
  bridgeBegin();
}
...
// Internal DRAM specifically: with PSRAM in the malloc pool the total free heap
// is megabytes and would never trip the guard, but internal DRAM is still what
// the SPI driver and mbedTLS run out of.
...
bool builderAdd(uint8_t type, uint8_t preset) {
  uint32_t heap = freeInternal();
  if (heap < MIN_FREE_HEAP) {
    printf("ADD refused, %u bytes free and %u needed\n", (unsigned)heap, (unsigned)MIN_FREE_HEAP);
    return false;
  }
  ...
  // Named before the slot counts as occupied. A slot that has never been used
  // still holds a zeroed preset byte, and letting that count as a taken name
  // would quietly retire the first preset in the list.
...
void builderReset() {
  nvs_erase_key(s_nvs, SLOTS_KEY);
  nvs_commit(s_nvs);
  printf("RESET slots cleared, restarting\n");
...
// Centred inside a box rather than measured by the caller: the 8x8 font is a
// third wider per character than the one the layout was drawn against, so no
// text position that matters survives being written out as a constant.
static void drawIn(int16_t x, int16_t w, int16_t y, const char *text, uint8_t size,
                   uint16_t colour) {
  gfxText(x + (w - gfxTextWidth(text, size)) / 2, y, text, colour, size);
}

static void drawCentred(int16_t y, const char *text, uint8_t size, uint16_t colour) {
  drawIn(0, PANEL_W, y, text, size, colour);
}
...
static void drawPageDots(uint8_t current) {
  ...
  for (uint8_t page = 0; page < pages; page++) {
    gfxFillCircle(x + page * spacing, PANEL_H - 44, 4, page == current ? COL_WHITE : COL_OUTLINE);
  }
}
...
static void drawChrome(uint8_t current) {
  if (s_screen == SCREEN_GRID) {
    gfxText(14, 24, "Accessories", COL_WHITE, 2);
    gfxFillCircle(PANEL_W - 24, 30, 7, bridgeCommissioned() ? COL_OK : COL_ALERT);
    char hint[40];
    snprintf(hint, sizeof(hint), "BOOT +/- PWR  %s", builderActiveLevelLabel());
    drawCentred(PANEL_H - 22, hint, 1, COL_FAINT);
  } else {
    gfxText(16, 24, "< back", COL_MUTED, 2);
...
static void drawAccessoryCells(uint8_t page, int16_t dx) {
  ...
  for (uint8_t i = first; i < tileCount() && i < first + PER_PAGE; i++) {
    ...
    if (i >= count) {
      gfxDrawRoundRect(x, y, TILE_W, TILE_H, 12, COL_OUTLINE);
      drawIn(x, TILE_W, y + 34, "+", 4, COL_MUTED);
      continue;
    }

    uint8_t slot = used[i];
    gfxFillRoundRect(x, y, TILE_W, TILE_H, 12, COL_BG);
    gfxDrawRoundRect(x, y, TILE_W, TILE_H, 12, accentFor(slot));
    gfxText(x + 12, y + 18, builderLabel(slot), COL_WHITE, 2);

    char detail[28];
    builderDescribe(slot, detail, sizeof(detail));
    gfxText(x + 12, y + 62, detail, COL_MUTED, 1);
  }
}
...
static void drawTypeCells(uint8_t page, int16_t dx) {
  ...
  for (uint8_t i = first; i < ACC_TYPE_COUNT && i < first + PER_PAGE; i++) {
    ...
    gfxFillRoundRect(x, y, TILE_W, TILE_H, 12, COL_BG);
    gfxDrawRoundRect(x, y, TILE_W, TILE_H, 12, accentForType(i));
    drawIn(x, TILE_W, y + 42, accessoryTypeName(i), 2, COL_WHITE);
  }
}
...
static void drawNameCells(uint8_t page, int16_t dx) {
  ...
  for (uint8_t i = first; i < count && i < first + PER_PAGE; i++) {
    ...
    gfxFillRoundRect(x, y, TILE_W, TILE_H, 12, COL_BG);
    gfxDrawRoundRect(x, y, TILE_W, TILE_H, 12, COL_OUTLINE);
    drawIn(x, TILE_W, y + 42, PRESETS[available[i]], 2, COL_WHITE);
  }
}
...
// Wide enough that "tap to confirm" clears the right edge in the 8x8 font.
static const int16_t REMOVE_X = PANEL_W - 124;
static const int16_t REMOVE_W = 124;

static void drawDetail(uint8_t slot) {
  gfxFillScreen(COL_BLACK);
  gfxText(16, 24, "< back", COL_MUTED, 2);
  drawIn(REMOVE_X, REMOVE_W, 26, s_confirmRemove ? "tap to confirm" : "remove", 1,
  ...
  switch (builderUi(slot)) {
    case UI_PRESS:
      gfxFillRoundRect(44, 150, 280, 190, 24, accentFor(slot));
      drawCentred(232, "PRESS", 3, COL_BLACK);
      return;
    case UI_ONOFF: {
      bool on = builderOnOff(slot);
      gfxFillRoundRect(44, 150, 280, 190, 24, on ? accentFor(slot) : COL_WELL);
      drawCentred(232, on ? "ON" : "OFF", 3, on ? COL_BLACK : COL_MUTED);
      return;
    }
    case UI_FLAG: {
      bool active = builderFlag(slot);
      gfxFillRoundRect(44, 150, 280, 190, 24, active ? accentFor(slot) : COL_WELL);
      drawCentred(232, flagLabel(builderType(slot), active), 3, active ? COL_BLACK : COL_MUTED);
      return;
    }
    case UI_VALUE: {
      gfxFillRoundRect(30, 200, 90, 120, 16, COL_WELL);
      drawIn(30, 90, 244, "-", 4, COL_WHITE);
      gfxFillRoundRect(248, 200, 90, 120, 16, COL_WELL);
      drawIn(248, 90, 244, "+", 4, COL_WHITE);
      char reading[16];
      formatValue(slot, reading, sizeof(reading));
  ...
  gfxFillRoundRect(30, 180, 90, 120, 16, COL_WELL);
  drawIn(30, 90, 224, "-", 4, COL_WHITE);
  gfxFillRoundRect(248, 180, 90, 120, 16, COL_WELL);
  drawIn(248, 90, 224, "+", 4, COL_WHITE);

  gfxDrawRoundRect(134, 180, 100, 120, 12, COL_OUTLINE);
  int16_t fill = (int16_t)((uint32_t)level * 116 / 255);
  gfxFillRoundRect(136, 180 + 2 + (116 - fill), 96, fill, 10, accentFor(slot));
  ...
  bool on = builderOnOff(slot);
  gfxFillRoundRect(60, 356, 248, 56, 16, on ? accentFor(slot) : COL_WELL);
...
static void touchDetail(uint8_t slot, int16_t x, int16_t y) {
  ...
  if (inRect(x, y, REMOVE_X, 4, REMOVE_W, 52)) {
    if (confirming) {
      builderRemove(slot);
    } else {
