...
// PATCHED — local override of IDF v5.5.5 esp_lcd, sole change (see ../PATCH.md).
// A transaction that completed with a DMA fault flag (PSRAM underflow/overflow,
// spi_master.c spi_trans_dma_error_check) comes back ESP_ERR_INVALID_STATE, but
// its result HAS been consumed from the return queue. The stock drain loops
// bailed out without decrementing num_trans_inflight for it, leaving the count
...
static esp_err_t panel_io_spi_del(esp_lcd_panel_io_t *io)
{
    ...
    for (size_t i = 0; i < num_trans_inflight; i++) {
        ret = lcd_spi_recycle_trans(spi_panel_io->spi_dev, &spi_trans);
